...
void 		ft_handle_label_declaration(char *str, size_t *index, t_valid* valid)
{
	...
	if (ft_strchr(str, LABEL_CHAR) && !(i = 0))
	{
		...
		printf("YO TEMP IS OVER___________HERE:%s\n", temp);
	}
	if (ft_strchr(temp, COMMENT_CHAR) || ft_strchr(temp, COMMENT_CHAR2) || ft_strchr(temp, LABEL_CHAR) == NULL)
	{
		...
		*index += ft_check_label_chars(str, distance, valid);
	}
	else
	{
		printf("ALOHA!\n");
		while (str[i] && str[i] != LABEL_CHAR)
		{
			...
			// else if (!ft_strchr(LABEL_CHARS, str[i])) // additional not-allowed chars -> #,."lol"
				...
				(str[i + 1] == LABEL_CHAR || str[i + 1] == '\0') ? ft_lexical_err(valid->line_num, i, valid) : 0; // some allowed chars -> ,' ''\t' // ft_lexical_err(valid->line_num, i, valid); 
			i++;
		}
...
void 		ft_handle_label_invocation(char *str, size_t *index, t_valid* valid)
{
	...
	while (str[i])
	{
		...
		{
			printf("AND CURRENT ERROR IS - %c | %zu\n", str[i], i);
			ft_lexical_err(valid->line_num, i, valid);
		}
		i++;
	}
	printf("ALREADY ADDED! | %zu | %zu\n", *index, i);
	*index += i;
...
void		ft_check_str_chars(char *str, size_t i, t_valid *valid)
{
	...
	{
		printf("YO, Line is in da club:%s\n", str);
		i += (str[i] == LABEL_CHAR) ? 1 : 0;
		(str[i] == LABEL_CHAR) ? ft_handle_label_invocation(str + i, &i, valid) :  ft_handle_label_declaration(str + i, &i, valid);
		printf("STRING after HANDLING:%s\n", str + i);
	}
