...
{
	// printf("STRING TO BE HANDLED AS ARGUMENT:%s\n", str);
	if (str[0] == 'r' && ft_isdigit(str[1]))
		return (1);
	else if ((str[0] == DIRECT_CHAR && str[1] == LABEL_CHAR) || (str[0] == DIRECT_CHAR && ft_isdigit(str[1])))
		return (2);
	else if ((str[0] == '-' && ft_isdigit(str[1])) || ft_isdigit(str[0]) || str[0] == LABEL_CHAR)
		return (4);
	else
		return (0);
}
